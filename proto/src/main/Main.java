...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("Jegmezo prototipus -- nullpointer csapat");

		String input = "";

		while (((input = scn.nextLine()).equals("") == false) && Controller.instance().getGameOn()) {
			if (input.length() > 0) {
				if (Controller.instance().isPolarBearManualMove())
					...
					for (int i = 1; i < numberOfParameters; i++) {
						parameters.add(splittedInput[i]);
					}
					executeTask(command, parameters);
				}
			}
		}
	}
	...
	private static void commandGameStart(ArrayList<String> parameters) {
		...
		int sizeOfMap = Integer.parseInt(parameters.get(0));
		parameters.remove(0);

		map = new Map(sizeOfMap, sizeOfMap);
		Field startField = map.getStartField();

		team = new Team(map);
		teamCreate(parameters, startField, team);
	}
